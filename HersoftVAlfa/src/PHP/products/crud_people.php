...
// require_once('people.php');
	class CrudPersona{
		...
		public function mostrar(){
			$db=Db::conectar();
			$listaPersona=[];
			$select=$db->query('SELECT * FROM publication');
            
			foreach($select->fetchAll() as $persona){
				$myPersona= new Persona();
				$myPersona->setId($persona['ID_Publication']);
				$myPersona->setNombre($persona['Name_product']);
				$myPersona->setdescription($persona['Description']);
				$myPersona->setPrice($persona['Price']);	
				$myPersona->setcategory($persona['category']);	
				$myPersona ->setdates($persona['dates']);
